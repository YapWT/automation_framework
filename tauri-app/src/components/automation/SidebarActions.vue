  ...
  <aside class="sidebar-left" :class="{ 'is-collapsed': collapsed }">
    ...
    <div class="scroll-area">
      ...
      <div class="category">
        ...
        <div class="action-item" @click="emit('add-step', 'fill')" title="Input Text (Ctrl + 2)">
          <Type ::size="collapsed ? 20 : 18" /> <span v-if="!collapsed" class="truncate">Input Text</span>
        </div>
        <div class="action-item" @click="emit('add-step', 'click')" title="Click Element (Ctrl + 3)">
          <MousePointer2 ::size="collapsed ? 20 : 18" /> <span v-if="!collapsed" class="truncate">Click Element</span>
        </div>
        <div class="action-item" @click="emit('add-step', 'wait_for')" title="Wait for Element (Ctrl + 4)">
          <Clock ::size="collapsed ? 20 : 18" /> <span v-if="!collapsed" class="truncate">Wait for Element</span>
        </div>
        <div class="action-item" @click="emit('add-step', 'upload')" title="Upload File (Ctrl + 5)">
          <Upload ::size="collapsed ? 20 : 18" /> <span v-if="!collapsed" class="truncate">Upload File</span>
        </div>
        <div class="action-item" @click="emit('add-step', 'download')" title="Download File (Ctrl + 6)">
      ...
      <div class="category">
        ...
        <div class="action-item" @click="emit('add-step', 'mkdir')" title="Create Folder (Ctrl + Shift + 2)">
          <FolderPlus ::size="collapsed ? 20 : 18" /> <span v-if="!collapsed" class="truncate">Create Folder</span>
        </div>
        <div class="action-item" @click="emit('add-step', 'move')" title="Move File (Ctrl + Shift + 3)">
      ...
      <div class="footer-actions mt-auto">
        ...
        <button v-if="!workflow.config.useExcel" class="xl-btn-base xl-sidebar-btn" @click="emit('select-excel')"
          title="Link Excel">
          <FileSpreadsheet ::size="collapsed ? 20 : 18" /> <span v-if="!collapsed" class="ml-3 truncate">Link
            Excel</span>
        </button>
        ...
        <div v-else class="xl-btn-base xl-sidebar-active" @click="emit('select-excel')">
          ...
          <XCircle v-if="!collapsed" :size="14" class="xl-clear ml-2 shrink-0"
            @click.stop="workflow.config.useExcel = false" />
        </div>

        <button class="reset-designer-btn mt-2" @click="emit('reset-designer')" title="Reset Designer (Ctrl + R)">
...
.reset-designer-btn:hover {
  background: #fee2e2;
}

.is-collapsed .xl-btn-base,
.is-collapsed .reset-designer-btn {
  justify-content: center;
  padding: 0;
}
