...
const props = defineProps<{ 
  ...
  clipboardStep: any,
  copiedSourceId: number | null // Added to show hint on the source card
}>();
...
<style scoped>
.designer-container { width: 100%; display: flex; flex-direction: column; align-items: center; position: relative; }
.drag-list-wrapper { width: 100%; max-width: 650px; padding: 20px 0; }
.drag-list { width: 100%; }

.step-wrapper { width: 100%; }

.inline-paste-btn {
  height: 8px; margin: 4px 0; border-radius: 6px; display: flex; align-items: center; justify-content: center;
  font-size: 0.65rem; font-weight: 800; color: #6366f1; text-transform: uppercase; cursor: pointer;
  opacity: 0; transition: all 0.2s; background: transparent; border: 1px dashed transparent;
}
.inline-paste-btn:hover { height: 32px; opacity: 1; background: #f5f3ff; border-color: #c4b5fd; margin: 8px 0; }

.step-card { 
  background: white; padding: 1.25rem; border-radius: 12px; display: flex; align-items: center; 
  gap: 1rem; border: 1px solid #e2e8f0; cursor: pointer; transition: 0.2s; position: relative;
  margin-top: 0.5rem; margin-bottom: 0.5rem;
}

/* ACTIVE STATE */
.step-card.active { border-color: #6366f1; box-shadow: 0 4px 12px rgba(99, 102, 241, 0.08); z-index: 2; }

/* COPIED SOURCE HINT STATE */
.step-card.is-copied {
  border: 1px dashed #6366f1;
  background: #f8faff;
}
.step-card.is-copied::after {
  content: "COPIED"; position: absolute; top: -8px; right: 12px; background: #6366f1;
  color: white; font-size: 10px; font-weight: 900; padding: 2px 6px; border-radius: 4px;
}

.step-badge { 
  width: 26px; height: 26px; background: #f1f5f9; border-radius: 50%; 
...
.paste-controls { position: sticky; bottom: 20px; display: flex; align-items: center; gap: 8px; z-index: 100; margin-top: 1.5rem; }

.paste-fab { 
  background: #f5f3ff; color: #7c3aed; border: 1px dashed #7c3aed; padding: 10px 24px; 
  border-radius: 12px; cursor: pointer; font-size: 0.8rem; font-weight: 700; display: flex; 
  align-items: center; gap: 8px; box-shadow: 0 4px 12px rgba(124, 58, 237, 0.1);
}

.cancel-fab {
  background: #fee2e2; color: #ef4444; border: 1px solid #fecaca; width: 38px; height: 38px;
  border-radius: 12px; display: flex; align-items: center; justify-content: center; cursor: pointer; transition: 0.2s;
}
.cancel-fab:hover { background: #ef4444; color: white; }
