...
import { 
  Play, Save, Plus, Trash2, Globe, FileSpreadsheet, MousePointer2, Copy, Clipboard,
  Type, Download, Move, FolderPlus, FileCode, Settings2, Zap, Info, GripVertical, Upload, Keyboard, Clock, Terminal, ChevronDown, RotateCcw, FolderSearch
...
const strategiesByAction: Record<string, any[]> = {
  fill: [{ id: 'textbox', label: 'Input / Text Box' }, { id: 'placeholder', label: 'By Placeholder' }, { id: 'label', label: 'By Label' }, { id: 'css', label: 'Advanced CSS' }],
...
// DEEP COPY FIX: Ensures pasted steps don't share data with originals
...
function handlePaste() {
  if (clipboardStep.value) {
    // Deep clone the clipboard object again to ensure unique references for every paste
    const newStep = JSON.parse(JSON.stringify(clipboardStep.value));
    newStep.id = Date.now(); // Unique ID
...
async function selectExcel() {
  ...
  if (selected) { workflow.value.config.excelPath = selected as string; workflow.value.config.useExcel = true; }
}

async function refreshSaved() { savedScripts.value = await invoke('list_saved_scripts'); }

async function loadScript(file: string) {
  const content = await invoke('read_script_content', { filename: file });
  manualCode.value = content; isManualEdit.value = true; activeTab.value = 'preview'; workflow.value.name = file.replace('.ts', '');
}

function addStep(type: string) {
  isManualEdit.value = false;
  const defaults: any = {
    navigate: { url: 'https://', timeout: 60000 },
...
async function handleRun(isTemp: boolean) {
  ...
  try { await invoke('execute_script', { filename: name }); } catch (err) { logs.value.push("Error: " + err); }
}
  ...
  <div class="editor-layout">
    ...
    <aside class="sidebar-left">
      ...
      <div class="category mt-auto pt-4 border-t">
         <button class="reset-btn-designer" @click="workflow.steps = []"><RotateCcw size="14"/> Reset Designer</button>
      </div>
    </aside>
    ...
    <main class="main-canvas">
      <header class="toolbar">
        <div class="tab-switcher">
          ...
          <button :class="{ active: activeTab === 'preview' }" @click="activeTab = 'preview'">Code View</button>
        ...
        <div class="controls-row">
          ...
          <div class="button-group">
            <button @click="showConsole = !showConsole" class="tool-btn-icon" :class="{ active: showConsole }" title="Toggle Console">
              <Terminal size="18"/>
            </button>
            <button @click="handleRun(true)" class="btn-primary test-btn"><Zap size="14"/> Test Run</button>
            <button @click="invoke('save_permanent_script', { code: finalCode, filename: workflow.name }).then(refreshSaved)" class="btn-primary save-btn">
      ...
      <section v-if="activeTab === 'editor'" class="canvas-content">
        <div v-if="workflow.steps.length === 0" class="empty-state">
          ...
          <p>Add steps from the left sidebar to start building.</p>
        </div>
          ...
          <template #item="{ element, index }">
            <div class="step-card" :class="{ active: selectedStepIndex === index }" @click="selectedStepIndex = index">
              ...
              <div class="step-tools">
                <button class="step-tool-btn" @click.stop="handleCopy(element)" title="Copy"><Copy size="14"/></button>
                <button class="step-tool-btn del" @click.stop="workflow.steps.splice(index, 1)" title="Delete"><Trash2 size="14"/></button>
              </div>
            </div>
          </template>
        </draggable>
        
        <button v-if="clipboardStep" class="paste-float-btn" @click="handlePaste">
    ...
    <aside class="sidebar-right">
      <div v-if="activeStep" class="prop-container">
        ...
        <div v-if="strategiesByAction[activeStep.action]" class="input-group">
          ...
          <div class="check-row mt-3" v-if="activeStep.params.exact !== undefined">
            ...
            <label for="ex-chk">Case Sensitive</label>
          </div>
        ...
        <div v-if="activeStep.params.index !== undefined" class="input-group">
          <label>Element Index (0 = First Found)</label>
        ...
        <div v-for="(val, key) in activeStep.params" :key="key">
          <div v-if="!['matchBy', 'exact', 'force', 'timeout', 'index'].includes(key)" class="input-group">
            ...
            <!-- Conditional Select for Key Press -->
            ...
            <textarea v-else v-model="activeStep.params[key]" rows="4" class="styled-textarea" placeholder="Value or {{ColumnName}}"></textarea>
          </div>
        </div>
        
        <div class="tip-box" v-pre><Info size="14" /><span>Use <b>{{ColumnName}}</b> for data mapping.</span></div>
      </div>
      ...
      <div class="sidebar-footer">
        <button class="excel-link-btn" @click="selectExcel">
          <FileSpreadsheet size="16" /> {{ workflow.config.useExcel ? 'Excel Connected' : 'Connect Excel' }}
        </button>
      </div>
    </aside>
  </div>
...
.action-item { display: flex; align-items: center; gap: 10px; padding: 10px; border-radius: 8px; cursor: pointer; font-size: 0.85rem; transition: 0.2s; color: #334155; }
...
.task-name-input { border: 1px solid #e2e8f0; border-radius: 8px; padding: 8px 12px; font-size: 0.85rem; width: 160px; outline: none; transition: 0.2s; }
.task-name-input:focus { border-color: #6366f1; }

.btn-primary { border: none; height: 36px; padding: 0 14px; border-radius: 8px; font-size: 0.8rem; font-weight: 700; cursor: pointer; display: flex; align-items: center; gap: 6px; color: white; }
.test-btn { background: #f59e0b; }
.save-btn { background: #6366f1; }
.tool-btn-icon { background: #f1f5f9; border: 1px solid #e2e8f0; border-radius: 8px; width: 36px; height: 36px; display: flex; align-items: center; justify-content: center; cursor: pointer; color: #64748b; }
.tool-btn-icon.active { background: #334155; color: white; }
...
.drag-list { width: 100%; max-width: 600px; }
.step-card { background: white; padding: 1.25rem; border-radius: 12px; display: flex; align-items: center; gap: 1rem; margin-bottom: 0.75rem; border: 1px solid #e2e8f0; cursor: pointer; transition: 0.2s; }
...
.step-type { font-weight: 800; font-size: 0.7rem; color: #94a3b8; display: block; text-transform: uppercase; margin-bottom: 2px; }
...
.action-tag { background: #f1f5f9; color: #6366f1; font-size: 0.65rem; padding: 2px 8px; border-radius: 20px; font-weight: 800; }
...
/* FOOTER */
.sidebar-footer { padding-top: 1.25rem; margin-top: auto; }
.excel-link-btn { width: 100%; padding: 12px; border: 1px dashed #cbd5e1; border-radius: 10px; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 8px; font-weight: 700; color: #64748b; background: #f8fafc; transition: 0.2s; font-size: 0.8rem; }
.reset-btn-designer { border: 1px solid #fee2e2; color: #ef4444; background: #fef2f2; padding: 10px; border-radius: 8px; cursor: pointer; font-size: 0.75rem; font-weight: 700; display: flex; align-items: center; justify-content: center; gap: 6px; width: 100%; }
...
.paste-float-btn { margin-top: 1.5rem; background: #f5f3ff; color: #7c3aed; border: 1px dashed #7c3aed; padding: 10px 24px; border-radius: 12px; cursor: pointer; font-size: 0.8rem; font-weight: 700; display: flex; align-items: center; gap: 8px; }
